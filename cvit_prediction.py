...
def faceforensics(
    ...
    count = 0
    accuracy = 0
    model = load_cvit(cvit_weight, fp16=False)
    model.float()
...
def predict(
    ...
    if fp16 and torch.cuda.is_available() and len(df) > 0:
        print("[DEBUG] Converting input tensor to float16 for half-precision on GPU.")
        df = df.half()
    elif len(df) > 0:
        print("[DEBUG] Input tensor remains in float32.")
        df = df.float()
...
def gen_parser():
    ...
    if fp16 and not torch.cuda.is_available():
        print("[WARNING] fp16 enabled but no GPU detected. Defaulting to full precision.")
...
def main():
    ...
    file_path = os.path.join("result", f"prediction_{dataset}_{net}.json")
